...
class System {
     ...
     * 入参date：日期对象/
     */
    transferDateToWeek(date) {
        let newDate = typeof date == "string" ? this.getDate(date) : date;
        return "日一二三四五六".charAt(new Date(newDate).getDay());
    }
    ...
    transferTimeFormat = (time) => {
        ...
        if (time && time.split(":").length == 2) {
            let arr = time.split(":");
            data = new Date(2018, 0, 1, Number(arr[0]), Number(arr[1]))
        }
        return data;
    }
